...
public partial class AutoScaffold : Page
{
    ...
    private string _manyToManyMap = String.Empty;
    ...
    //Set to true to display ProductID as Product for end user
    private const bool CLEAN_COLUMN_NAMES = false;

    //private TableSchema.Table Schema;

    private const string PROVIDER_NAME = "AS_PROVIDER";
    private const string TABLE_NAME = "AS_TABLE";
    //private HtmlTable tbl;
    ...
    private void SetupPage()
    {
        ...
        if (HttpContext.Current.Request.QueryString["id"] != null)
        {
            ...
            string keyID = HttpContext.Current.Request.QueryString["id"].ToString();
    ...
    private void SetTableName(TableSchema.Table tblSchema)
    {
    ...
    private void BuildTableList()
    {
        ...
        foreach (TableSchema.Table tblSchema in tables)
        {
            if (CodeService.ShouldGenerate(tblSchema.Name, tblSchema.Provider.Name))
            {
                ...
                if(tblSchema.PrimaryKey != null && tblSchema.PrimaryKeys.Length == 1)
                {
        ...
        if (tables.Length > 0 && String.IsNullOrEmpty(TableName))
        {
            ...
            {
                if (tables[i].PrimaryKey != null && tables[i].PrimaryKeys.Length == 1 && CodeService.ShouldGenerate(tables[i].Name, tables[i].Provider.Name))
                {
    ...
    private static HtmlTableRow AddRow(HtmlTable tbl, string text, int colspan)
    {
    ...
    /// <param name="control"></param>
    private static HtmlTableRow AddRow(HtmlTable tbl, string cellValue1, Control control)
    {
        HtmlTableRow tr = new HtmlTableRow();
        tbl.Rows.Add(tr);
    ...
    private void SaveManyToMany(string mapTableName)
    {
        ...
        if (fkField != string.Empty)
        {
            ...
            string sql = "INSERT INTO " + mapTableName + "(" + fkField + "," + Schema.PrimaryKey.ColumnName + ") VALUES (" + Utility.PrefixParameter("fk", Schema.ProviderType) + "," + Utility.PrefixParameter("pk)", Schema.ProviderType);
            ...
            if (chk != null)
            {
                foreach (ListItem item in chk.Items)
                {
                    if (item.Selected)
                    {
                        ...
                        cmd.Parameters.Add(Utility.PrefixParameter("fk", Schema.ProviderType), item.Value, DbType.Int32);
                        cmd.Parameters.Add(Utility.PrefixParameter("pk", Schema.ProviderType), pk);
    ...
    private void SaveEditor()
    {
        ...
        if (pk != null)
        {
            ...
            foreach(TableSchema.TableColumn col in Schema.Columns)
            {
                ...
                string colParameter = Utility.PrefixParameter(col.ParameterName, Schema.ProviderType);
            ...
            if (col.DataType != DbType.Binary && col.DataType != DbType.Byte)
            {
                if(col.IsPrimaryKey)
                {
                    if(!col.AutoIncrement && String.IsNullOrEmpty(col.DefaultSetting))
                    {
                        ...
                        paramList += Utility.PrefixParameter(col.ParameterName, schema.ProviderType) + ",";
                    }
    ...
    private static string BuildUpdateSql(TableSchema.Table schema)
    {
        ...
        foreach (TableSchema.TableColumn col in schema.Columns)
        {
            if (col.DataType != DbType.Binary && col.DataType != DbType.Byte)
            {
                ...
                {
                    sql += col.ColumnName + " = " + Utility.PrefixParameter(col.ParameterName, schema.ProviderType) + ",";
                }
            }
        }

        sql = sql.Remove(sql.Length - 1, 1);
        sql += " WHERE " + schema.PrimaryKey.ColumnName + " = " + Utility.PrefixParameter(schema.PrimaryKey.ColumnName, schema.ProviderType);
        return sql;
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        //{
        //    pk = ((Label)tbl.FindControl("pkID")).Text;
        //}
        //else
        //{
        //    pk = ((TextBox)tbl.FindControl("pkID")).Text;
        //}
        string pk = null;
        Control c = tblEditor.FindControl("pkID");
        if (c is Label)
